...
class Model_CNN_NDC:
    def __init__(self):
        ...
        price_vec_model = Sequential([
            Convolution2D(88, 5, price_vec_dim, border_mode='valid',
                          ...
                          activity_regularizer=activity_l2(0.005),
        ...
        vol_model = Sequential([
            Convolution2D(88, 5, vol_dim, border_mode='valid',
        ...
        rmsprop = RMSprop(lr=0.0005, rho=0.9, epsilon=1e-08, decay=0.01)
        sgd = SGD(lr=0.0005, decay=1e-6, momentum=0.5, nesterov=True)

        self._model.compile(optimizer=sgd,  # adadelta
                            loss='mse',
    ...
    def train(self, training_set, validation_set, test_set):
        ...
        while loss >= 0.5:  # 0.02:
            ...
            print('\nTesting ------------')
            loss, accuracy = self._model.evaluate(X_test, y_test, batch_size=batch_size)
